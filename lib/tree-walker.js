...
var getTrees = function(main, config, treeCache){
  return builder.loadConfig(config.systemConfig).then(function(){
    var inverseIndex = {};
    var treeIndex = {};
    var trees = [];

    // allow override of the baseURL from system.config.js
    if (config.baseURL) {
    ...
    var buildDeps = function(src, level){
      ...
      return builder.tracer.traceModule(src).then(function(traceTree){
