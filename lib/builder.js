...
var build = function (config) {
  ...
  return builder.loadConfig(config.jspmConfigPath, undefined, true).then(function() {
    return treeWalker.getTrees(config.main, config, treeCache, builder).then(function (tree) {
      appTree = tree;

      config.routes.forEach(function (route) {
        promises.push(new RSVP.Promise(function (resolve, reject) {
          treeWalker.getTrees(route, config, treeCache, builder).then(function (tree) {
      ...
      return RSVP.all(promises).then(function () {
        ...
        return buildTrees(bundles, config, builder);
      });
    }, function (error) {
...
var buildTrees = function (bundles, config, builder) {
  var builderPromises = [];
  ...
  Object.keys(bundles).forEach(function (bundleName) {
    builderPromises.push(new RSVP.Promise(function (resolve, reject) {
      buildTree(bundles[bundleName], "bundles/" + bundleName, config, builder).then(function () {
        resolve();
      }, function (error) {
        console.log(error);
  ...
  routeTrees.forEach(function (treeIndex) {
    Object.keys(treeIndex).forEach(function (moduleName) {
      builderPromises.push(new RSVP.Promise(function (resolve, reject) {
        buildTree(treeIndex[moduleName], moduleName, config, builder).then(function () {
          resolve();
        }, function (error) {
          console.log(error);
          console.log(error.stack)
  ...
  Object.keys(appTree).forEach(function (moduleName) {
    builderPromises.push(new RSVP.Promise(function (resolve, reject) {
      ...
      buildTree(appTree[moduleName], moduleName, config, builder).then(function () {
        if (isMainModule) {
          if (config.verboseOutput){
...
var buildTree = function (tree, destination, config, builder) {
  ...
  if (config.verboseOutput){
    console.log('\t\t building ' + config.dest + '/' + destination);
  }
